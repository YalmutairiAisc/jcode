        ...
        match build_hook_process(&command_line, &event) {
            Ok(mut cmd) => {
                ...
                match crate::platform::spawn_detached_no_window(&mut cmd) {
                    Ok(child) => {
                        // Reap it (upstream 2026-08): a detached hook process
                        // that is never waited on becomes a zombie. The
                        // _no_window spawn is ours: without CREATE_NO_WINDOW a
                        // console flashes on Windows for every single tool call.
                        // Both fixes are needed and they compose.
