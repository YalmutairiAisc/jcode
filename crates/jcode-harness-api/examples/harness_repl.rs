...
// Unix-only on purpose. Both the real transport (`UnixStream::connect`) and
// the scripted demo (`UnixStream::pair`) are Unix sockets, and this file is a
// minimal protocol reference rather than a product surface -- the supported
// cross-platform client is `jcode-sdk`, whose transport (`jcode-transport`)
// is a named pipe on Windows. Before this gate the example failed to COMPILE
// on Windows, which broke `cargo test --workspace` before a single test ran,
// and with it `selfdev test` on Windows machines (found 2026-08-18 while
// investigating why self-dev was unhealthy there). One `cfg` over a private
// module beats a dozen per-item gates that must never drift apart.
#[cfg(unix)]
mod unix_only {
...
fn main() {
    eprintln!(
        "harness_repl is a Unix-socket protocol reference; on Windows use the          jcode-sdk crate, whose transport is a named pipe."
    );
}
